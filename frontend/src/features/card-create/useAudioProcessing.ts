...
/**
 * 音频选择与处理链 hook（创建模式）
 * 收敛 audioFile/createAudioFiles/audioOptions/processing/processProgress
 */
export function useAudioProcessing() {
  ...
  /** 选定音频文件：首个作为主音频，全部进入多选列表（拖拽与文件选择共用） */
   ...
   */
  const processIfNeeded = useCallback(async (file: File): Promise<{ file: File; durationSec: number }> => {
    // 未开启处理：直接测量原文件时长
    ...
    try {
      const processed = await processAudio(file, audioOptions, setProcessProgress)
      let durationSec = 0
