...
// PublicBaseURL returns the absolute base URL for publicly readable objects:
// the CDN domain when configured, otherwise the bucket endpoint.
func (s *COSStorage) PublicBaseURL() string {
	if s.cdnURL != "" {
		return s.cdnURL
	}
	return s.baseURL
